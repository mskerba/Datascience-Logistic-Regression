...
def binary_target(df, house_name):
    ...
    for epoch in range(max_epoch):
        indices = np.arange(len(matrix_with_ones))
        np.random.shuffle(indices)
        X_shuffled = matrix_with_ones[indices]
        y_shuffled = house[indices]

        for i in range(len(X_shuffled)):
            xi = X_shuffled[i]            # one sample (1 x n)
            yi = y_shuffled[i]            # its label
            ...
            grad = (hi - yi) * xi
            thetas = thetas - learning_rate * grad   # update after each sample
